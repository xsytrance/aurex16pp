...
fn main() {
    ...
    for id in 0..game_controller.num_joysticks().unwrap_or(0) {
        ...
        match game_controller.open(id) {
            ...
            Err(e) => {
                eprintln!("Controller open failed for id {id}: {e}");
            }
        }
    }
    ...
    'running: loop {
        ...
        let mut pad_jump = false;

        if let Some(c) = &controller {
            let lx = c.axis(Axis::LeftX);
            pad_left = lx < -8_000 || c.button(Button::DPadLeft);
            pad_right = lx > 8_000 || c.button(Button::DPadRight);
            pad_jump = c.button(Button::A) || c.button(Button::B) || c.button(Button::X);

            if !started && (pad_left || pad_right || pad_jump) {
                started = true;
                system.start_game();
                println!("Starting tech demo...");
            }
        ...
        let input = if started {
            InputState {
                ...
                right: kb.is_scancode_pressed(Scancode::Right)
                    ...
                    || pad_right,
                jump: kb.is_scancode_pressed(Scancode::Space)
